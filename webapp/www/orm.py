...
class Field(object):
    ...
    # 当打印(数据库)表时，输出(数据库)表的信息:类名（列名），字段类型和名字
    def __str__(self):
...
# 所有的元类都继承自type
# ModelMetaclass元类定义了所有Mode l基类(继承ModelMetaclass)的子类实现的操作
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for k,v in attrs.items():
            ...
            if isinstance(v, Field):
                #此处打印的k是类的一个属性（列名），v是这个属性在数据库中对应的Field列表属性（列属性）
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    async def findNumber(cls, selectField, where=None, args=None):
        ...
        #__num__是什么鬼？sql中的别名？mysql可以省略as?
